...
public class Wrappers {
    ...
    abstract static class _LedgerIndexBound extends Wrapper<Long> implements Serializable {
        ...
        private void checkAdditionOverflow(Long addedValue) {
            Preconditions.checkArgument(FluentCompareTo.is(Long.MAX_VALUE - addedValue).greaterThanEqualTo(this.value()), String.format("Value too large. Adding %s would cause an overflow.", addedValue));
        }

        @Value.Auxiliary
        private void checkSubtractionInBounds(Long subtractedValue) {
            Preconditions.checkArgument(FluentCompareTo.is(subtractedValue).lessThanOrEqualTo(this.value() - 1), String.format("Value too large. Subtracting %s would result in a LedgerIndexBound below 1.", subtractedValue));
        }
    }
}
